...
def test_financial_summary_parsing():
    ...
    summary = parse_qbo_to_financial_summary(pl_data, bs_data)
    ...
    empty_summary = parse_qbo_to_financial_summary({}, {})
...
def test_burn_rate_edge_cases():
    ...
    res1 = calculate_burn_rate(empty_summary)
    ...
    res2 = calculate_burn_rate(single_month)
...
def test_runway_edge_cases():
    ...
    res_warn = calculate_runway(summary, burn_warning)
    ...
    res_monitor = calculate_runway(summary, burn_monitor)
    ...
    res_overflow = calculate_runway(summary, burn_infinity)
    assert res_overflow.cash_zero_date is None

def test_cash_forecast_empty():
    empty_summary = FinancialSummary(current_cash_balance=Decimal("0"), monthly_financials=[])
    res = calculate_cash_forecast(empty_summary)
